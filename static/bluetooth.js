...
window.nativeBluetoothResult = function (ok, message) {
  const statusEl = document.getElementById("illuminate-error");
  const button = document.getElementById("illuminate-button");
...
function illuminateClimbViaNativeBridge(board, bluetoothPacket) {
...
function illuminateClimb(board, bluetoothPacket) {
  ...
  if (hasNativeBluetoothBridge()) {
    illuminateClimbViaNativeBridge(board, bluetoothPacket);
    return;
  }
  ...
  requestDevice(capitalizedBoard)
    ...
    .then(() => window.nativeBluetoothResult(true, null))
    .catch((error) => {
      if (error.message === BLUETOOTH_CANCELLED) {
        window.nativeBluetoothResult(true, null); // silent - user just closed the picker
        return;
      }
      const message =
      ...
      window.nativeBluetoothResult(false, message);
    });
}
